...
export const DyadCodebaseContext: React.FC<DyadCodebaseContextProps> = ({
  ...
  return (
    <div
      className={`relative rounded-xl px-4 py-2 my-2 cursor-pointer glass-surface transition-colors ring-1 ${
        inProgress
          ? "ring-[rgba(237,50,121,0.35)] shadow-[0_0_0_3px_rgba(237,50,121,0.06)]"
    ...
    >
      ...
      <div
        className="absolute top-2 left-2 flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold bg-white/90 dark:bg-zinc-900/80 shadow-sm"
        style={{ zIndex: 1 }}
      >
        <Code2 size={16} style={{ color: "#ed3279" }} />
        <span className="glass-contrast-text">Nati Codebase Context</span>
      </div>

      {/* File count when collapsed */}
      {files.length > 0 && (
        <div className="absolute top-2 left-40 flex items-center">
          <span className="px-1.5 py-0.5 rounded text-xs glass-surface glass-hover text-gray-700 dark:text-gray-300">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {files.map((file, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="px-2 py-1 rounded-lg glass-surface"
                  >
                    <div className="flex items-center gap-1.5">
                      ...
                      />
                      <div className="text-sm font-medium glass-contrast-text">
                        {fileName}
                      </div>
                    </div>
                    {pathPart && (
