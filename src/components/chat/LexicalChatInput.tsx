...
export function LexicalChatInput({
  ...
  useEffect(() => {
    const onFiles = (e: Event) => {
      const detail = (e as CustomEvent).detail as { files?: string[] } | undefined;
      if (detail?.files && Array.isArray(detail.files)) {
        setProjectFiles(detail.files);
      }
    };
    window.addEventListener("filetree:files", onFiles as EventListener);
    return () => window.removeEventListener("filetree:files", onFiles as EventListener);
  }, []);

  // Fallback: fetch files from IPC when FileTree isn't mounted
  useEffect(() => {
    let cancelled = false;
    (async () => {
      ...
      try {
        ...
        if (!cancelled && app?.files) setProjectFiles(app.files);
      } catch {
        // ignore
      }
    })();
  ...
  // Plugin: receive chat:mention-file events and insert @path
  function MentionFromFileTreePlugin() {
    const [editor] = useLexicalComposerContext();
    useEffect(() => {
      const handler = (e: Event) => {
        ...
        if (!path) return;
        // Bring focus to the chat input automatically
        editor.focus();
        editor.update(() => {
          const root = $getRoot();
          ...
          if ($isRangeSelection(selection)) {
            // Insert a BeautifulMention node for styled @path
            const needsLeadingSpace = !root.getTextContent().endsWith(" ") && root.getTextContent().length > 0;
            const nodes = [] as any[];
            if (needsLeadingSpace) nodes.push($createTextNode(" "));
            nodes.push($createBeautifulMentionNode("@", path));
            nodes.push($createTextNode(" "));
            selection.insertNodes(nodes);
          } else {
            const p = $createParagraphNode();
            p.append($createBeautifulMentionNode("@", path));
