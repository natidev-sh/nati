...
export function AppUpgrades({ appId }: { appId: number | null }) {
  ...
  if (isLoading) {
    return (
      <div className="mt-6">
        <h3 className="text-lg font-semibold mb-3 glass-contrast-text">
          App Upgrades
        </h3>
    ...
    <div className="mt-6">
      ...
      {currentUpgrades.length === 0 ? (
        <div
          ...
          className="p-4 glass-surface border rounded-lg text-sm text-green-800 dark:text-green-300 border-green-300 dark:border-green-800/50"
        >
          Nati is up to date with all features enabled.
        </div>
      ) : (
        <div className="space-y-4">
          {currentUpgrades.map((upgrade: AppUpgrade) => (
            <div
              key={upgrade.id}
              className="p-4 glass-surface border rounded-lg flex justify-between items-start border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow"
            >
              <div className="flex-grow">
                <h4 className="font-semibold glass-contrast-text">
                  {upgrade.title}
                </h4>
                ...
                {mutationError && upgradingVariables === upgrade.id && (
                  <Alert variant="destructive" className="mt-3 glass-surface">
                      ...
                      <a
                        onClick={(e) => {
                          ...
                          IpcClient.getInstance().openExternalUrl(
                            upgrade.manualUpgradeUrl ?? "https://natidev.com/docs",
                          );
                        }}
