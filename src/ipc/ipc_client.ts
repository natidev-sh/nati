...
export class IpcClient {
  ...
  public async readAppFileDataUrl(appId: number, filePath: string): Promise<string> {
    return this.ipcRenderer.invoke("read-app-file-data-url", {
      appId,
      filePath,
    });
  }
  ...
  public streamMessage(
    ...
    if (attachments && attachments.length > 0) {
      // Process each file attachment and convert to base64 (downscale large images)
      ...
      const maybeDownscaleImage = async (file: File): Promise<string> => {
        try {
          if (!file.type.startsWith("image/")) {
            return await toDataUrl(file);
          }
          // Create image
          const imgDataUrl = await toDataUrl(file);
          const img = new Image();
          ...
          // Prefer JPEG for photos, PNG otherwise
          const isPng = file.type.includes("png");
          const mime = isPng ? "image/png" : "image/jpeg";
          const quality = isPng ? 0.92 : 0.85;
          const downscaled = canvas.toDataURL(mime, quality);
          return downscaled;
        } catch {
      ...
      Promise.all(
        attachments.map(async (attachment) => {
          const dataUrl = await maybeDownscaleImage(attachment.file);
          return {
  ...
  public async renameFile(params: {
    appId: number;
    oldPath: string; // relative to app root
    newPath: string; // relative to app root
  }): Promise<void> {
    await this.ipcRenderer.invoke("filetree:rename-file", params);
  }
  ...
  public async deleteFile(params: {
    ...
    await this.ipcRenderer.invoke("filetree:delete-file", params);
  }

  public async revealInFolder(params: {
    appId: number;
    filePath: string; // relative to app root
  }): Promise<void> {
    await this.ipcRenderer.invoke("filetree:reveal-in-folder", params);
  }
  ...
  // --- Supabase DB Browser ---
  public async getSupabaseSchema(appId: number): Promise<any> {
    return this.ipcRenderer.invoke("supabase:get-schema", { appId });
  }

  public async executeSupabaseSql(appId: number, sql: string): Promise<any> {
    return this.ipcRenderer.invoke("supabase:execute-sql", { appId, sql });
  }
  // --- End Supabase DB Browser ---
  ...
  // --- Backups ---
  public async createBackup(reason?: string): Promise<{ path: string }> {
    return this.ipcRenderer.invoke("backup:create", reason);
  }
  ...
  public async deleteBackup(name: string): Promise<{ success: true }> {
    return this.ipcRenderer.invoke("backup:delete", name);
  }

  public async getBackupSize(name: string): Promise<{ size: number }> {
    return this.ipcRenderer.invoke("backup:size", name);
  }
