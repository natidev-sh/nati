...
`;
  ...
  // Serve files (e.g., images/SVG) as data URLs
  ipcMain.handle(
    "read-app-file-data-url",
    async (
      _,
      ...
      const appRoot = await fsPromises.realpath(appPath);
      const safeRel = filePath.replace(/^[/\\]+/, "");
      const fullPath = path.resolve(appRoot, safeRel);
      ...
      // For SVG, prefer utf8 to preserve text and avoid base64 bloat
      if (mime === "image/svg+xml") {
        try {
          const svgText = await fsPromises.readFile(fullPath, "utf-8");
          return `data:image/svg+xml;utf8,${encodeURIComponent(svgText)}`;
        } catch {
          // fallback to base64 if utf8 read fails
        }
      }
...
export function registerAppHandlers() {
  ...
  ipcMain.handle(
    "filetree:rename-file",
    async (
      _,
      { appId, oldPath, newPath }: { appId: number; oldPath: string; newPath: string },
    ) => {
      const app = await db.query.apps.findFirst({ where: eq(apps.id, appId) });
      if (!app) throw new Error("App not found");
      const appPath = getDyadAppPath(app.path);
      const fullOld = path.join(appPath, oldPath);
      const fullNew = path.join(appPath, newPath);
      if (!fullOld.startsWith(appPath) || !fullNew.startsWith(appPath)) {
        throw new Error("Invalid file path");
      }
  ...
  // Delete a file within the app directory
  ipcMain.handle(
    ...
    async (
      ...
      if (!app) throw new Error("App not found");
      const appPath = getDyadAppPath(app.path);
      const full = path.join(appPath, filePath);
      if (!full.startsWith(appPath)) {
        throw new Error("Invalid file path");
      }
  ...
  ipcMain.handle(
    ...
    async (_, { appId, filePath }: { appId: number; filePath: string }) => {
      ...
      const appPath = getDyadAppPath(app.path);
      const appRoot = await fsPromises.realpath(appPath);
      // Normalize potential leading slash in filePath and resolve safely
      const safeRel = filePath.replace(/^[/\\]+/, "");
      const fullPath = path.resolve(appRoot, safeRel);
      // Security: ensure resolved path stays within app root
      if (!fullPath.startsWith(appRoot)) {
        throw new Error("Invalid file path");
      }
