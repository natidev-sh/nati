    ...
    try {
      ...
      if (req.attachments && req.attachments.length > 0) {
        ...
        for (const [index, attachment] of req.attachments.entries()) {
          ...
          const dataUrl = attachment.data; // keep full data URL for client previews
          ...
          if (attachment.attachmentType === "upload-to-codebase") {
            ...
            const fileId = `NATI_ATTACHMENT_${index}`;
            ...
            // Also add a display tag so the UI can render a preview
            attachmentInfo += `<dyad-attachment name="${attachment.name}" type="${attachment.type}" data-url="${dataUrl}"></dyad-attachment>\n`;
          } else {
            // For chat-context, use the existing logic
            attachmentInfo += `- ${attachment.name} (${attachment.type})\n`;
            // Add a display tag for UI preview
            attachmentInfo += `<dyad-attachment name="${attachment.name}" type="${attachment.type}" data-url="${dataUrl}"></dyad-attachment>\n`;
