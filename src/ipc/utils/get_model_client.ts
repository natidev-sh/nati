...
export async function getModelClient(
  ...
  // Handle Nati Pro override
  if (dyadApiKey && settings.enableDyadPro) {
    // Check if the selected provider supports Nati Pro (has a gateway prefix) OR
    ...
    if (providerConfig.gatewayPrefix != null || dyadEngineUrl) {
      ...
      logger.warn(
        `Nati Pro enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
